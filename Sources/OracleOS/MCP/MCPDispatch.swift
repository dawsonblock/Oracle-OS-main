...
public enum MCPDispatch {
    ...
    private static func dispatch(request: MCPToolRequest) async -> ToolResult {
        ...
        case "oracle_architecture_review":
            ...
            if let reviewDict = toDictionary(review) {
                return ToolResult(success: true, data: ["review": reviewDict])
            }
    ...
    // MARK: - Type-Safe Serialization
    // Helper to convert Codable types to [String: Any] for response formatting
    // (output path only). Never use in input path.
